...
public class Board {
	...
	private boolean canBlockRook(boolean isBlack){
		...
		if (!isBlack){
			if (threateningRow != blackKingRow && threateningColumn == blackKingColumn){
				...
				for (int i = min + 1 ; i < max ; i++){
					if (canBlockIn(i,blackKingColumn, !isBlack))
						return true;
				}
				...
				for (int j = min + 1 ; j < max ; j++){
					if (canBlockIn(blackKingRow,j, !isBlack))
						return true;
				}
			}
		}else{
			if (threateningRow != whiteKingRow && threateningColumn == whiteKingColumn){
				...
				for (int i = min + 1 ; i < max ; i++){
					if (canBlockIn(i,whiteKingColumn, !isBlack))
						return true;
				}
				...
				for (int j = min + 1 ; j < max ; j++){
					if (canBlockIn(whiteKingRow,j, !isBlack))
						return true;
				}
			}
		}
	...
	private boolean canBlockIn(int r2, int c2, boolean isBlack){
		if (!isBlack){
			/*peon*/
			if (( (r2 +1) < 8 && board[r2 +1][c2]!=null && board[r2 +1][c2].equals("P")) ||
				( (r2 +2) == 6 && board[r2+2][c2]!=null && board[r2+2][c2].equals("P"))	)
				return true;
		}else{
			/*peon*/
			if (( (r2 - 1) > 0 && board[r2 -1][c2]!=null && board[r2 -1][c2].equals("p")) ||
				( (r2 -2) == 1 && board[r2-2][c2]!=null && board[r2-2][c2].equals("p"))	)
					return true;
		}
